...
| 阶段 | 里程碑 ⓪✅ → ①✅ → ②✅（`milestone-2`）→ ③✅（`milestone-3`）→ ④ 受限传输 → ⑤–⑦ |
...
规格与步骤级计划在 `README.md` §13。每个任务以可测交付物结束，走 TDD，**一个任务一次提交**（祈使句英文 commit）。当前：里程碑 ⓪✅、①✅、②✅、③✅（`milestone-3`）；下一步 ④ 受限传输（udp + http1）。

**③**（已完成）：3.1∥3.5 → 3.2∥3.3 → 3.4∥3.7 → 3.6∥3.8 → 3.9 grpc-go 双向矩阵。

**④ 的推荐并行切分**：

```
4.1 transport/udp ──▶ 4.2 envelope UDP 批量 + 尺寸校验
4.3 transport/http1 ──▶ 4.4 framing/wholebody
```

- **可并行**：udp 链 ∥ http1 链（仅依赖 ①/②）。
- **④ 也可与 ⑤ 部分并行**（⑤ 依赖接口 + 至少一个可跑协议）。
