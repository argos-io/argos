...
规格与步骤级计划在 `README.md` §13。每个任务以可测交付物结束，走 TDD，**一个任务一次提交**（祈使句英文 commit）。当前：⓪–④✅（`milestone-4`）；下一步 ⑤ 代码生成与 example。

**④**（已完成）：4.1∥4.3 → 4.2∥4.4；交付 udp、http1、envelope×udp、wholebody×http1。

**⑤ 的推荐并行切分**：

```
5.1 IR FullName/Shape ──▶ 5.2 描述符模板 ──▶ 5.3 服务端模板 ∥ 5.4 客户端模板
                                              └──────────────▶ 5.5 example/echo
```

- 代码生成工具若尚未迁入本分支，先摸清仓库内 gen 相关路径再动手。
