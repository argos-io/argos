...
func newRESTRouter(bindings []Binding) (restRouter, error) {
	if len(bindings) == 0 {
		return restRouter{}, fmt.Errorf("framing/httpunary: REST requires at least one Binding")
	}
	byMethod := make(map[string]Binding, len(bindings))
	seen := make(map[string]struct{}, len(bindings))
	for _, b := range bindings {
		if b.Method.IsZero() {
			return restRouter{}, fmt.Errorf("framing/httpunary: binding has zero Method")
		}
		verb := strings.ToUpper(strings.TrimSpace(b.Verb))
		if verb == "" {
			return restRouter{}, fmt.Errorf("framing/httpunary: binding %q missing Verb", b.Method.FullName())
		}
		pat := normalizePattern(b.Pattern)
		if pat == "" {
			return restRouter{}, fmt.Errorf("framing/httpunary: binding %q missing Pattern", b.Method.FullName())
		}
		key := verb + " " + pat
		if _, ok := seen[key]; ok {
			return restRouter{}, fmt.Errorf("framing/httpunary: duplicate binding %q", key)
		}
		seen[key] = struct{}{}
...
func (r restRouter) BuildPreface(m descriptor.Method, codecName string, out metadata.Metadata) (transport.RequestPreface, error) {
	...
	if !ok {
		return transport.RequestPreface{}, status.Error(status.Internal,
			fmt.Sprintf("framing/httpunary: no REST binding for method %q", m.FullName()))
	}
...
func buildPath(pattern string, out metadata.Metadata) (string, error) {
	...
	for i, seg := range segs {
		...
		if name == "" {
			return "", status.Error(status.InvalidArgument, "framing/httpunary: empty path variable name")
		}
		...
		if val == "" {
			return "", status.Error(status.InvalidArgument,
				fmt.Sprintf("framing/httpunary: missing path variable %q in metadata", name))
		}
		b.WriteByte('/')
		b.WriteString(val)
	}
...
func (r restRouter) ResolveAccept(httpMethod, requestTarget string, in metadata.Metadata) (string, error) {
	...
	if len(pathMatches) == 0 {
		return "", fmt.Errorf("%w: %w", framing.ErrCallRejected,
			status.Error(status.NotFound, "framing/httpunary: route not found"))
	}
	...
	return "", fmt.Errorf("%w: %w", framing.ErrCallRejected,
		status.Error(status.Unimplemented, "framing/httpunary: http method not allowed"))
}
