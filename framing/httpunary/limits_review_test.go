...
func openLimitedCall(t *testing.T, cfg framing.Config, body []byte, hdr map[string]string) (framing.ClientSession, framing.Call) {
	t.Helper()
	...
	call, err := cs.OpenCall(ctx, descriptor.MustMethod("svc.Limits", descriptor.Unary),
		framing.CallSpec{Metadata: metadata.New(metadata.RoleInitiator, nil)})
	if err != nil {
...
func TestOversizeResponseKeepsSessionReusable(t *testing.T) {
	t.Parallel()
	cs, call := openLimitedCall(t, framing.Config{MaxMessageSize: 16}, make([]byte, 4096), nil)
...
func TestOversizeResponseMetadataKeepsSessionReusable(t *testing.T) {
	...
	cs, call := openLimitedCall(t,
		framing.Config{MaxMessageSize: 1 << 20, MaxInboundMetadataSize: 32},
		[]byte("ok"),
...
func TestOversizeSendReportsResourceExhausted(t *testing.T) {
	t.Parallel()
	_, call := openLimitedCall(t, framing.Config{MaxMessageSize: 16}, []byte("ok"), nil)
