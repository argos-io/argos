...
func DecodeTimeout(s string) (time.Duration, error) {
	size := len(s)
	if size < 2 {
		return 0, fmt.Errorf("framing/grpc: timeout string too short: %q", s)
	}
	if size > 9 {
		return 0, fmt.Errorf("framing/grpc: timeout string too long: %q", s)
	}
	unit := timeoutUnit(s[size-1])
	d, ok := timeoutUnitToDuration(unit)
	if !ok {
		return 0, fmt.Errorf("framing/grpc: timeout unit not recognized: %q", s)
	}
	t, err := strconv.ParseUint(s[:size-1], 10, 64)
	if err != nil {
		return 0, fmt.Errorf("framing/grpc: malformed timeout %q: %w", s, err)
	}
