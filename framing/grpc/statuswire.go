...
func httpFallbackStatus(httpStatus int) error {
	...
	if httpStatus == http.StatusOK {
		return status.Error(status.Unknown, "framing/grpc: missing grpc-status in response")
	}
	code := httpstatus.FromHTTP(httpStatus)
	return status.Error(code, fmt.Sprintf("framing/grpc: missing grpc-status; HTTP %d", httpStatus))
}
