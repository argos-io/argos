...
业务仍走 **http2 + grpc + protobuf** 三轴；Health、Reflection 是**额外 Register 的标准 gRPC 服务**，与 `echo.v1.EchoService` 等业务服务共用同一监听面（相同 `ServiceListenAddress` + 相同三轴工厂时，`server.Run` 只开一条 listener，按 method 路由）。

包路径（按需 import，不进入根包 `argos`）：

| 包 | 作用 |
|---|---|
| `framing/grpc/health` | `grpc.health.v1.Health`（Check / List / Watch） |
| `framing/grpc/reflection` | `grpc.reflection.v1.ServerReflection`（v1，对齐 grpc-go `RegisterV1`） |
| `framing/grpc/retry` | 客户端 `Attempts` 包装 Open（非内置重试策略） |
...
func main() {
	...
	srv := server.New(
		...
		// 与业务相同的三轴 + 相同地址 → 共用一个 http2 listener
...
func dialEcho(ctx context.Context) (*client.Client, error) {
	return client.New(
		...
		argos.JoinClient(
			argos.WithTransport(tr), argos.WithFraming(fr), argos.WithCodec(cd),
		),
...
默认重试 `Unavailable` 与 `DeadlineExceeded`；自定义 `Policy.Retriable` 即可。未实现 grpc-go ServiceConfig 透明重试 / pushback。

---

## 背压（budget）

无需额外 API：`MaxBufferedBytes` 与 `MaxConcurrentCalls` 已在 `Open` / `handleCall` 准入时 carve `perCall`，grpc / wholebody / envelope 在读写 payload 时扣减。超限为 `status.ErrCallsExhausted`（与 envelope 一致）。
...
| Health ↔ grpc-go client | `framing/grpc/health/health_test.go` |
| Reflection ListServices | `framing/grpc/reflection/reflection_test.go` |
| Retry Attempts | `framing/grpc/retry/retry_test.go` |
