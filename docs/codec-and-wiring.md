...
内置 `protobuf` / `json` 面向 `proto.Message`。wholebody 默认 JSON + protojson；grpc/envelope 默认 protobuf。类型与 codec 不匹配在**首次调用**失败（见 README §7）。
...
Framing **不**从全局 `Config` 读；客户端 per-call `budget` 在 call ctx 上（当前 mainly envelope 使用，见 README §4.2）。
