...
- **`MaxBufferedBytes`**：`perCall = MaxBufferedBytes / MaxConcurrentCalls` 放入调用 ctx 的 `budget.Budget`；`framing/grpc`、`framing/httpunary` 等在读写路径 `TryAcquire`。服务端在 `admit` 后通过 `framing.BudgetSetter` 晚绑定到 Call。
