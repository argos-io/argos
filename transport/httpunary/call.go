...
func checkInboundMeta(cfg session.Options, md metadata.Metadata) error {
	...
	if sz := metadata.WireSize(md); sz > max {
		return status.Error(status.ResourceExhausted,
			fmt.Sprintf("httpunary: inbound metadata %d exceeds limit %d", sz, max))
	}
	return nil
}
...
// deadline. Call.Close must unblock in-flight Recv/Send (session.Call), and
...
func (c *call) SendHeaders() error {
	if c.initiator {
		return status.Error(status.Unimplemented, "httpunary: SendHeaders unsupported for initiator")
	}
	return status.Error(status.Unimplemented,
		"httpunary: SendHeaders unsupported (UnaryResponseWriter commits at Finish)")
}
...
func (c *call) Send(payload []byte) error {
	...
	if c.initiator && c.reqBuf != nil {
		c.mu.Unlock()
		return status.Error(status.Internal, "httpunary: unary allows one request message")
	}
	if !c.initiator && c.respBuf != nil {
		c.mu.Unlock()
		return status.Error(status.Internal, "httpunary: unary allows one response message")
	}
	c.mu.Unlock()
...
func (c *call) HalfClose() error {
	if !c.initiator {
		return status.Error(status.Unimplemented, "httpunary: HalfClose unsupported for responder")
	}
	c.mu.Lock()
	if c.closed {
		c.mu.Unlock()
	...
	if !ok {
		return status.Error(status.Unimplemented, "httpunary: carrier has no CloseSend")
	}
...
func (c *call) Finish(err error) error {
	if c.initiator {
		return status.Error(status.Unimplemented, "httpunary: Finish unsupported for initiator")
	}
	c.mu.Lock()
	if c.closed {
		c.mu.Unlock()
	...
	if !ok {
		return status.Error(status.Unimplemented, "httpunary: carrier has no UnaryResponseWriter")
	}
...
func (c *call) recvClient() ([]byte, func(), error) {
	...
	if !ok {
		return nil, nil, fmt.Errorf("httpunary: carrier missing ResponseHeaderReader")
	}
...
func (c *serverCall) Accept(m descriptor.Method) error {
	if m.IsZero() {
		return status.Error(status.InvalidArgument, "httpunary: zero Method")
	}
...
func readAllLimited(r io.Reader, limit int64) ([]byte, error) {
	...
	if int64(len(b)) > limit {
		...
		return nil, status.Error(status.ResourceExhausted,
			fmt.Sprintf("httpunary: message exceeds max size %d", limit))
	}
	return b, nil
}

var (
	errConcurrentSend = fmt.Errorf("httpunary: concurrent Send")
	errConcurrentRecv = fmt.Errorf("httpunary: concurrent Recv")
	errSendFinished   = fmt.Errorf("httpunary: send already finished")
	errCallClosed     = fmt.Errorf("httpunary: call closed")
)
