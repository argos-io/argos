...
// Axis implements transport.Transport for unary HTTP over transport/http1: routing,
// concurrent reuse on one connection, and the client pool live here rather than
...
// WithCodecName sets the codec name passed in SessionSpec at dial (default "json").
...
// WithREST selects REST-style HTTP routing; default is RPC-style POST /Service/Method.
func WithREST(c RESTConfig) TransportOption {
	return func(a *Transport) {
		copy := c
		a.restCfg = &copy
	}
...
// NewTransport returns HTTP unary over http1 as transport.Transport.
func NewTransport(opts ...TransportOption) (*Transport, error) {
	...
	var fr session.Framing
	var err error
	if a.restCfg != nil {
		fr, err = newREST(*a.restCfg)
	} else {
		fr = newRPC()
	}
	if err != nil {
...
func init() {
	transport.Register("httpunary", func() (transport.Transport, error) { return NewTransport() })
}
...
// Shutdown drains the listener and in-flight connections on the http1 transport.
...
func (s *linkServerConn) AcceptCall(ctx context.Context, spec transport.CallSpec) (transport.ServerCall, error) {
	if s.sess == nil {
		return nil, fmt.Errorf("httpunary: AcceptCall before Handshake")
	}
