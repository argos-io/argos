...
func ParseMethodPath(target string) (service, method string, err error) {
	...
	if !strings.HasPrefix(target, "/") {
		return "", "", fmt.Errorf("httpunary: invalid method path %q: must start with /", target)
	}
	rest := target[1:]
	pos := strings.LastIndex(rest, "/")
	if pos < 0 {
		return "", "", fmt.Errorf("httpunary: invalid method path %q: missing /method", target)
	}
	service, method = rest[:pos], rest[pos+1:]
	if service == "" || method == "" {
		return "", "", fmt.Errorf("httpunary: invalid method path %q: empty service or method", target)
	}
