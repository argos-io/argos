...
func TestReuseConcurrent(t *testing.T) {
	if got := newRPC().Reuse(); got != session.Concurrent {
...
func TestUnaryEchoHTTP1(t *testing.T) {
	...
	fr := newRPC()
	spec := session.SessionSpec{CodecName: "json", Options: testSessionOptions()}
	...
	addr := startHTTP1(t, func(ctx context.Context, c transport.Conn) {
		...
		call, err := sess.AcceptCall(ctx, session.CallSpec{Metadata: md})
		if err != nil {
	...
	call, err := cliSess.OpenCall(context.Background(), method, session.CallSpec{Metadata: md})
	if err != nil {
...
func TestAcceptRejectsNonUnary(t *testing.T) {
	...
	call, err := cliSess.OpenCall(context.Background(), unary, session.CallSpec{Metadata: md})
	if err != nil {
		t.Fatalf("OpenCall: %v", err)
	}
	defer func() { _ = call.Close() }()
