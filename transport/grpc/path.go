...
func ParseMethodPath(path string) (service, method string, err error) {
	if !strings.HasPrefix(path, "/") {
		return "", "", fmt.Errorf("grpc: invalid method path %q: must start with /", path)
	}
	rest := path[1:]
	pos := strings.LastIndex(rest, "/")
	if pos < 0 {
		return "", "", fmt.Errorf("grpc: invalid method path %q: missing /method", path)
	}
	service, method = rest[:pos], rest[pos+1:]
	if service == "" || method == "" {
		return "", "", fmt.Errorf("grpc: invalid method path %q: empty service or method", path)
	}
