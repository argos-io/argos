...
func TestTrailersOnlyError(t *testing.T) {
	...
	go func() {
		...
		defer ss.Close()
		sc, err := ss.AcceptCall(ctx, session.CallSpec{
	...
	defer cs.Close()
	call, err := cs.OpenCall(ctx, method, session.CallSpec{
...
func TestHTTPFallbackMissingGrpcStatus(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	cs, err := f.NewClientSession(ctx, cli, session.SessionSpec{CodecName: "proto"})
	if err != nil {
		t.Fatal(err)
	}
	defer cs.Close()
...
func TestHTTPFallback200WithoutGrpcStatus(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	cs, err := f.NewClientSession(ctx, cli, session.SessionSpec{})
	if err != nil {
		t.Fatal(err)
	}
	defer cs.Close()
	call, err := cs.OpenCall(ctx, descriptor.MustMethod("svc.M", descriptor.Unary), session.CallSpec{
		Metadata: metadata.New(metadata.RoleInitiator, nil),
	})
	if err != nil {
		t.Fatal(err)
...
func TestPercentEncodedStatusMessage(t *testing.T) {
	...
	go func() {
		...
		ss, _ := f.NewServerSession(ctx, srvConn, session.SessionSpec{})
		defer ss.Close()
		sc, _ := ss.AcceptCall(ctx, session.CallSpec{
			Metadata: metadata.New(metadata.RoleResponder, nil),
		})
		_, _, _ = sc.Recv()
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	cs, _ := f.NewClientSession(ctx, cli, session.SessionSpec{})
	defer cs.Close()
	call, _ := cs.OpenCall(ctx, method, session.CallSpec{
		Metadata: metadata.New(metadata.RoleInitiator, nil),
	})
	defer call.Close()
	_ = call.HalfClose()
