...
func TestDecodeGrpcMessageToleratesBadPercent(t *testing.T) {
	...
	for _, in := range []string{"%zz", "%A", "ok%", "%%"} {
		got := DecodeGrpcMessage(in)
		if got != in {
			t.Errorf("Decode(%q) = %q, want unchanged", in, got)
		}
	}
	if got := DecodeGrpcMessage("%41"); got != "A" {
