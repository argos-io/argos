...
var errInvalidLPMFlag = errors.New("grpc: invalid LPM compressed flag")
...
var ErrLPMUnsynced = fmt.Errorf("grpc: LPM stream out of sync")
...
func ReadLPMLimited(r io.Reader, maxLen int64) (compressed bool, payload []byte, err error) {
	...
	if maxLen > 0 && int64(n) > maxLen {
		st := status.Error(status.ResourceExhausted,
			fmt.Sprintf("grpc: LPM length %d > max %d", n, maxLen))
