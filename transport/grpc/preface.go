...
func ParseRequestHeaders(target string, hs transport.Headers) (RequestInfo, error) {
	...
	for _, h := range hs {
		...
		case "grpc-timeout":
			...
			if err != nil {
				return RequestInfo{}, fmt.Errorf("grpc: malformed grpc-timeout: %w", err)
			}
			info.Timeout = d
		...
		default:
			...
			if err != nil {
				return RequestInfo{}, fmt.Errorf("grpc: malformed binary metadata %q in header %q: %w", h.Value, key, err)
			}
			md[key] = append(md[key], vals...)
		}
	}
	if contentType == "" {
		return RequestInfo{}, fmt.Errorf("grpc: missing content-type")
	}
	subtype, ok := ContentSubtype(contentType)
	if !ok {
		return RequestInfo{}, fmt.Errorf("grpc: invalid gRPC content-type %q", contentType)
	}
...
func DecodeResponseHeaders(hs transport.Headers) (md metadata.Metadata, contentSubtype string, encoding string, err error) {
	...
	for _, h := range hs {
		...
		default:
			...
			if derr != nil {
				return nil, "", "", fmt.Errorf("grpc: malformed binary metadata %q in header %q: %w", h.Value, key, derr)
			}
			md[key] = append(md[key], vals...)
		}
	}
	if contentType != "" {
		...
		if !ok {
			return nil, "", "", fmt.Errorf("grpc: invalid gRPC content-type %q", contentType)
		}
	}
