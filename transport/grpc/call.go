...
func (s *clientSession) OpenCall(ctx context.Context, m descriptor.Method, spec session.CallSpec) (session.Call, error) {
	if m.IsZero() {
		return nil, status.Error(status.InvalidArgument, "grpc: zero Method")
	}
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	...
	if s.closed || !s.reusable {
		s.mu.Unlock()
		return nil, status.Error(status.Unavailable, "grpc: session not reusable")
	}
	s.inFlight++
	s.mu.Unlock()
	...
	if !ok {
		_ = car.Abort()
		s.endFlight()
		return nil, fmt.Errorf("grpc: OpenStream Carrier is not ByteStreamCarrier")
	}
...
func checkInboundMeta(cfg session.Options, md metadata.Metadata) error {
	...
	if sz := metadata.WireSize(md); sz > max {
		return status.Error(status.ResourceExhausted,
			fmt.Sprintf("grpc: inbound metadata %d exceeds limit %d", sz, max))
	}
	return nil
}
...
func (s *serverSession) AcceptCall(ctx context.Context, spec session.CallSpec) (session.ServerCall, error) {
	...
	if !ok {
		return nil, fmt.Errorf("grpc: server Carrier missing RequestHeaderReader")
	}
	bs, ok := s.carrier.(transport.ByteStreamCarrier)
	if !ok {
		return nil, fmt.Errorf("grpc: server Carrier is not ByteStreamCarrier")
	}
	...
	if err != nil {
		...
		return newServerCall("", s.subtype, "", sendComp, time.Time{}, false),
			fmt.Errorf("%w: %w", session.ErrCallRejected,
	...
	if err := checkInboundMeta(s.cfg, info.Metadata); err != nil {
		...
		return newServerCall(fullName, s.subtype, "", sendComp, time.Time{}, false),
			fmt.Errorf("%w: %w", session.ErrCallRejected, err)
	}
	if spec.Metadata != nil {
...
func (c *call) SendHeaders() error {
	if c.initiator {
		return status.Error(status.Unimplemented, "grpc: SendHeaders unsupported for initiator")
	}
	c.mu.Lock()
	if c.closed {
		c.mu.Unlock()
...
func (c *call) writeResponseHeadersOnce() error {
	c.headersOnce.Do(func() {
		...
		if !ok {
			c.headersErr = status.Error(status.Unimplemented, "grpc: carrier has no ResponseWriter")
			return
		}
...
func (c *call) Send(payload []byte) error {
	...
	if c.maxMsg > 0 && int64(len(data)) > c.maxMsg {
		return status.Error(status.ResourceExhausted,
			fmt.Sprintf("grpc: Send payload %d > max %d", len(data), c.maxMsg))
	}
	...
	if c.cfg.MaxFrameSize > 0 && int64(len(wire)) > c.cfg.MaxFrameSize {
		return status.Error(status.ResourceExhausted,
			fmt.Sprintf("grpc: Send wire payload %d > max frame %d", len(wire), c.cfg.MaxFrameSize))
	}
...
func (c *call) HalfClose() error {
	if !c.initiator {
		return status.Error(status.Unimplemented, "grpc: HalfClose unsupported for responder")
	}
	c.mu.Lock()
	if c.closed {
		c.mu.Unlock()
	...
	if !ok {
		return status.Error(status.Unimplemented, "grpc: carrier has no CloseSend")
	}
...
func (c *call) Finish(err error) error {
	if c.initiator {
		return status.Error(status.Unimplemented, "grpc: Finish unsupported for initiator")
	}
	c.mu.Lock()
	if c.closed {
		c.mu.Unlock()
	...
	rw, ok := c.carrier.(transport.ResponseWriter)
	if !ok {
		return status.Error(status.Unimplemented, "grpc: carrier has no ResponseWriter")
	}
...
func (c *call) Recv() (payload []byte, release func(), err error) {
	...
	if compressed {
		...
		return nil, nil, status.Error(status.ResourceExhausted,
			fmt.Sprintf("grpc: Recv payload %d > max %d", len(data), c.maxMsg))
	}
	c.mu.Lock()
	c.messagesRecv = true
	c.mu.Unlock()
...
func (c *serverCall) Accept(m descriptor.Method) error {
	if m.IsZero() {
		return status.Error(status.InvalidArgument, "grpc: zero Method")
	}
...
var (
	errConcurrentSend = fmt.Errorf("grpc: concurrent Send")
	errConcurrentRecv = fmt.Errorf("grpc: concurrent Recv")
	errSendFinished   = fmt.Errorf("grpc: send already finished")
	errCallClosed     = fmt.Errorf("grpc: call closed")
)
