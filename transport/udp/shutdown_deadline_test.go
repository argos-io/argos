...
func TestShutdownBoundedByDeadlineWhenCallbackBlocks(t *testing.T) {
	...
	// A datagram creates the server-side association and its onConn callback.
	if err := conn.(transport.CarrierConn).Carrier().(transport.DatagramCarrier).SendDatagram([]byte("hi")); err != nil {
